...
export default function Home() {
  ...
  const joinGame = async () => {
    ...
    try {
      const joinedRoom = await client.joinOrCreate("clever", { name: playerName });
      setRoom(joinedRoom);
      setIsJoined(true);
      ...
      if (joinedRoom.state) {
        setGameState(joinedRoom.state.toJSON());
      }
    } catch (e) {
    ...
    <main className="p-8 max-w-6xl mx-auto text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-6">
          ...
          <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
            <div className="space-y-3">
              {gameState.phase === 'WAITING' && (
                <button className="w-full bg-green-600 hover:bg-green-500 py-4 rounded-xl font-bold shadow-lg" onClick={() => room?.send("start")}>START GAME</button>
              )}
              ...
              {!isActiveTurn && gameState.phase === 'PASSIVE_CHOOSING' && (
                <div className="p-4 bg-indigo-500/10 border border-indigo-500/30 rounded-xl text-center text-xs text-indigo-400 font-bold">
                  PASSIVE PHASE: Choose from Platter
                </div>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-8">
                ...
                <div>
                  ...
                  <div className="flex gap-2 min-h-[64px] p-2 bg-black/10 rounded-xl overflow-x-auto">
                    {gameState.silverPlatter?.map((die: any) => (
                      <button
                        key={die.id}
                        disabled={isActiveTurn || gameState.phase !== 'PASSIVE_CHOOSING'}
                        onClick={() => room?.send("finishPassiveTurn", { dieId: die.id })}
                        className={`w-11 h-11 rounded-lg flex items-center justify-center text-lg font-bold shadow-md transition-transform ${getDieColorClass(die.color)} ${!isActiveTurn && gameState.phase === 'PASSIVE_CHOOSING' ? 'hover:scale-110 ring-2 ring-white/20' : 'opacity-40 grayscale'}`}
                      >
                        {die.value}
                      </button>
                    ))}
          ...
          {/* My Scoring Sheet */}
          {myPlayer && (
            <div className={`space-y-6 transition-all duration-500 ${gameState.phase === 'MARKING' || gameState.phase === 'BONUS_MARKING' ? 'ring-4 ring-blue-500/40 p-6 rounded-3xl bg-blue-500/5' : 'opacity-80'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <YellowArea data={myPlayer.sheet.yellow} onMark={(row, col) => room?.send("mark", { area: 'yellow', row, col, isBonus: gameState.phase === 'BONUS_MARKING' })} />
                <BlueArea data={myPlayer.sheet.blue} onMark={(val) => room?.send("mark", { area: 'blue', value: val, isBonus: gameState.phase === 'BONUS_MARKING' })} />
              </div>
              <div className="space-y-4">
                <GreenArea data={myPlayer.sheet.green} onMark={() => room?.send("mark", { area: 'green', isBonus: gameState.phase === 'BONUS_MARKING' })} />
                <OrangeArea data={myPlayer.sheet.orange} onMark={() => room?.send("mark", { area: 'orange', isBonus: gameState.phase === 'BONUS_MARKING' })} />
                <PurpleArea data={myPlayer.sheet.purple} onMark={() => room?.send("mark", { area: 'purple', isBonus: gameState.phase === 'BONUS_MARKING' })} />
              </div>
            </div>
          )}
        </div>
